...
type PageState = 'info' | 'success' | 'already_paid'
    ...
    return (
      <div className="min-h-screen bg-gray-950">
        ...
        <div className="mx-auto max-w-sm px-4 py-16 text-center">
          ...
          <Link to={`/s/${slug}`} className="inline-block mt-6 text-sm px-4 py-2 rounded-lg bg-brand-700/30 text-brand-300 hover:bg-brand-700/50 border border-brand-700/40 transition-colors">
            View group
          </Link>
        </div>
      </div>
    ...
    return (
      <div className="min-h-screen bg-gray-950">
        ...
        <div className="mx-auto max-w-sm px-4 py-16 text-center">
          ...
          <Link to={`/s/${slug}/standings`} className="text-sm px-4 py-2 rounded-lg bg-brand-700/30 text-brand-300 hover:bg-brand-700/50 border border-brand-700/40 transition-colors">
            View standings
          </Link>
        </div>
      </div>
  ...
  if (state === 'success') {
    return (
      <div className="min-h-screen bg-gray-950">
        ...
        <div className="mx-auto max-w-sm px-4 py-16 text-center">
          <span className="text-5xl block mb-4">🎉</span>
          <h1 className="text-xl font-bold text-white mb-2">Payment Recorded!</h1>
          <p className="text-sm text-gray-400 mb-6">
            Your {offlineMethod === 'cashapp' ? 'CashApp' : 'Zelle'} payment of{' '}
            ...
            {info.allow_cashapp && (
              <div className="rounded-xl border border-emerald-800/40 bg-emerald-900/10 p-4">
                <div className="flex items-center justify-between mb-2">
                  <span className="text-sm font-medium text-emerald-300">💚 CashApp</span>
                  {info.cashapp_handle && (
                    <span className="text-sm font-bold text-white">{info.cashapp_handle}</span>
                  )}
                </div>
                <p className="text-xs text-gray-400 mb-3">
                  Send {fmt(parseFloat(info.amount))} to{' '}
                  {info.cashapp_handle ? <span className="text-white font-medium">{info.cashapp_handle}</span> : 'the organizer\'s CashApp'}
                  {' '}then click below.
                </p>
                <button
                  onClick={() => { setOfflineMethod('cashapp'); offlinePay.mutate('cashapp') }}
                  disabled={offlinePay.isPending}
                  className="w-full py-2.5 bg-emerald-800 text-emerald-100 font-semibold text-sm rounded-lg hover:bg-emerald-700 disabled:opacity-50 transition-colors"
                >
                  {offlinePay.isPending && offlineMethod === 'cashapp' ? 'Recording…' : 'I\'ve sent the payment'}
                </button>
              </div>
            )}

            {info.allow_zelle && (
              <div className="rounded-xl border border-indigo-800/40 bg-indigo-900/10 p-4">
                <div className="flex items-center justify-between mb-2">
                  <span className="text-sm font-medium text-indigo-300">🔵 Zelle</span>
                  {info.zelle_handle && (
                    <span className="text-sm font-bold text-white">{info.zelle_handle}</span>
                  )}
                </div>
                <p className="text-xs text-gray-400 mb-3">
                  Send {fmt(parseFloat(info.amount))} via Zelle to{' '}
                  {info.zelle_handle ? <span className="text-white font-medium">{info.zelle_handle}</span> : 'the organizer\'s Zelle'}
                  {' '}then click below.
                </p>
                <button
                  onClick={() => { setOfflineMethod('zelle'); offlinePay.mutate('zelle') }}
                  disabled={offlinePay.isPending}
                  className="w-full py-2.5 bg-indigo-800 text-indigo-100 font-semibold text-sm rounded-lg hover:bg-indigo-700 disabled:opacity-50 transition-colors"
                >
                  {offlinePay.isPending && offlineMethod === 'zelle' ? 'Recording…' : 'I\'ve sent the payment'}
                </button>
              </div>
            )}
          </div>
