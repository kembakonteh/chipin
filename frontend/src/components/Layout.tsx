...
function buildTabs(features: { campaigns_enabled: boolean; susu_enabled: boolean; org_enabled: boolean } | null): Tab[] {
  ...
  if (features?.campaigns_enabled) tabs.push({ to: '/dashboard', label: 'Campaigns', icon: '📋' })
  if (features?.susu_enabled) tabs.push({ to: '/susu', label: 'Susu', icon: '💰' })
  if (features?.org_enabled && !features?.campaigns_enabled) tabs.push({ to: '/orgs', label: 'My Org', icon: '👥' })
...
export default function Layout({ children }: { children: ReactNode }) {
  ...
  function isActive(to: string) {
    if (to === '/dashboard') return pathname === '/dashboard' || pathname.startsWith('/dashboard/') || pathname.startsWith('/campaigns/')
